...
mod icons {
    ...
    #[component]
    pub(super) fn toggle_dark_mode() -> Element {
    ...
    pub(crate) fn dropdown() -> Element {
        let dark_mode = use_context::<Signal<DarkMode>>();
        let mut is_open = use_signal(|| false);

        rsx! {
            div {
                class: "dropdown [--strategy:static] md:[--strategy:absolute] [--adaptive:none] md:[--trigger:hover] py-3 md:py-6",
                ...
                button {
                    ...
                    class: if dark_mode().0 { "text-neutral-200 hover:text-neutral-500" } else { "text-gray-800 hover:text-gray-600" },
                    onclick: move |event| {
                        info!(name: "onclick", "Clicked! Event: {:?}", event);
                        is_open.set(!is_open())
                    },
                    "\n                Company\n                "
                    svg {
                ...
                div {
                    class: "dropdown-menu transition-[opacity,margin] duration-[0.1ms] md:duration-[150ms] md:w-80 z-10  md:shadow-2xl rounded-lg py-2 md:p-2  before:absolute top-full before:-top-5 before:start-0 before:w-full before:h-5",
                    class: if is_open() { "block opacity-100" } else { "hidden opacity-0" },
                    class: if dark_mode().0 { "bg-neutral-800 divide-neutral-700" } else { "bg-white" },
                    style: if is_open() { "position: absolute; margin: 0px;" },
                    a {
                        href: "#",
